...
export default function VacanciesListingPage() {
  ...
  useEffect(() => {
    const fetchVacancies = async () => {
      try {
        ...
        const data: VacancyDto[] = await res.json();
        setVacancies(data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const filtered = vacancies.filter((v) => {
    const matchesSearch =
      ...
      v.startupResponseDto?.name?.toLowerCase().includes(search.toLowerCase());

    const matchesCategory =
      !categoryFilter || v.startupResponseDto?.category === categoryFilter;
      ...
      <section className="bg-gradient-to-b from-blue-50 to-white py-12 px-4 border-b border-gray-100">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">
            Bütün <span className="text-blue-600">vakansiyalar</span>
          </h1>
          <p className="text-gray-500 text-sm mt-1.5">
            Startuplardakı açıq iş elanlarını kəşf edin
          </p>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 mt-6">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              type="text"
              placeholder="Vakansiya və ya startup axtar..."
              value={search}
          ...
          <select
            ...
            onChange={(e) =>
              setCategoryFilter(e.target.value as StartupCategory | "")
            }
        ...
        {loading ? (
          ...
          <div className="text-center py-16 space-y-2">
            <p className="text-gray-400 text-sm">Heç bir vakansiya tapılmadı.</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {paginated.map((vacancy) => {
              const startup = vacancy.startupResponseDto;
              const color = getLogoColor(startup?.id ?? vacancy.id);
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      >
                        {startup?.name ? (
                          ...
                          <Briefcase className={`w-4 h-4 ${color.text}`} />
                        )}
                      </div>
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full inline-block ${
                              vacancy.isActive ? "bg-green-500" : "bg-gray-400"
                            }`}
                          />
